...
- If neither is set, the model runs with random weights (useful for sanity checks).
- Results are saved to the same `logs/` structure as a training run (metrics, visualizations, etc.). See the `logs-structure` skill for details.

## Defaults

Both `restore_ckpts` and `restore_dir` default to `null` (defined in `configs/paths/default.yaml`). Override them on the command line as shown above, or set them in `configs/train_custom.yaml` (git-ignored) for repeated use:
